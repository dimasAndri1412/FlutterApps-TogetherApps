...
class ClockInController {
  ...
  Future<String> fetchProject() async {
    ...
    if (userId != null) {
      var response = await http.get(
        Uri.parse(
          'http://192.168.100.84/FlutterAPI/GetUserById.php/$userId'
        )
      );
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
  ...
  try {
    ...
    var request = http.MultipartRequest(
      'POST',
      Uri.parse("http://192.168.100.84/FlutterAPI/attendance/user/clock_in.php"),
    );
  ...
  show_question() async {
    try {
      ...
      final response = await http.post(
        Uri.parse("http://192.168.100.84/FlutterAPI/KPI/getQuestionsForClockOut.php"),
        body: {
          'user_id' : userId
        }
      );
  ...
  clock_out() async {
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse("http://192.168.100.84/FlutterAPI/attendance/user/clock_out.php"),
      );
